...
- **Test data**: `TestDataController` (`POST /test-data/load`) is a plain `@ResponseBody` JSON-less endpoint (returns a status string, not a redirect) wired to the "Teszt adatok betöltése" button in the header fragment via `fetch` + `alert(...)`, so it works from any page. `MqTestDataService` connects with the hardcoded dev queue connection (see below, not the `MqConnectionModel` form) and MQ-PUTs a batch of generated sample JSON payloads onto the queue — it doesn't touch the local `message` table; a user still browses them in via the normal `POST /load` flow afterwards. Put failures are wrapped in `MqPutException` (mirrors `MqBrowseException`, but kept separate since put/browse are different failure domains).
